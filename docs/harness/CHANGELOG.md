...
1부 흐름(`/analyze-requirements` → `/design-db`)이 v2.4.11+ 안전망 훅을 **자동으로 발동시키는 구조**임을 활용하여 학습자가 직접 차단 메시지를 보고 안전망 가치를 체감할 수 있도록 슬라이드 추가.

- **`presentation-intro.html` SLIDE 8.5 신설** — "🛡️ 잠깐 — 안전망 자동 체험"
  - 시도 카드: `/design-db` 바로 호출
  - 자동 차단 카드: 훅이 출력하는 실제 차단 메시지 (빨강 배경)
  - 해결 카드: `/review-analyze` 한 줄 → reviewed/ 채워짐 → 진입 가능 (cyan)
  - 메시지: "⚙️ v2.4.11+ 코드 안전망 — 깜빡해도 시스템이 막아준다"
- 카운터 13 → 14 갱신

### 추가 — `onboarding.md` 강사 노트 신설

1부 흐름이 안전망 체험을 위해 `-all` 묶음 스킬을 의도적으로 안 쓰는 이유 명시:
- 교육 효과 (단계별 체험)
- `-all` 묶음은 review-* 안전망까지 자동 포함하므로 reviewed/ 자동 채워짐 → 차단 메시지를 볼 수 없음
- **유의**: 1부 흐름을 단순화하려는 시도가 안전망 체험을 사라지게 함

---

## v2.4.14 — 2026-05-20

### 수정 — 안전망 흐름도 가독성 대폭 개선 (그룹 묶음 + 안전망 강조)

기존 흐름도 약점:
- 박스 수 너무 많음 (신규 17, 운영 18)
- 안전망 vs 일반 단계 weight 차이 약함
- 그룹 묶음 시각화 부족
- 코드 강제 메커니즘이 흐름도에 안 보임

개선:
- **그룹 묶음 박스** (`.flow-group`) — Phase 그룹별로 묶고 안에 작은 단계 chip 표시
- **안전망 박스 강조** (`.flow-safety`) — 더 크게, cyan 진한 테두리 + glow + 좌상단에 "🟢⚙️ 안전망 (코드 강제)" 라벨
- 박스 수: 신규 17 → **11**, 운영 18 → **13**
- 그라데이션 배경 (cyan + 오렌지) 으로 안전망의 두 층위 시각적 표현
- 신규/운영 동일 형식 — 단 운영은 사전 / Phase 1~1.6 / CONTEXT 누적 그룹 추가

### 효과
- 한 화면에서 큰 흐름 파악 가능
- 안전망이 단계 사이마다 들어가는 게 시각적으로 명확
- 코드 강제(⚙️)가 안전망 박스 라벨에 직접 표시

---

## v2.4.13 — 2026-05-20

### 수정 — pipeline-overview html 의 ⚙️ 코드 강제 색상 분리

사용자 지적: 안전망(🟢 cyan) 과 코드 강제(⚙️ cyan) 가 같은 톤이라 구분 약함.

- CSS 변수 `--enforce: #ff9d44` (오렌지) 추가 — 빨강 대비 "에러" 의미 없이 강한 인상
- 범례의 "⚙️ 코드 강제" → `.badge-enforce` 클래스 적용 (오렌지 배경 + bold + 테두리)
- 표 안 ⚙️ 로 시작하는 td 셀에 **자동 강조 적용** (JS 가 페이지 로드 시 클래스 추가):
  - 배경: `#ff9d4416` (옅은 오렌지)
  - 좌측 보더: `3px solid var(--enforce)`
- 효과: 24개 강제 행이 표 안에서 시각적으로 즉시 식별

---

## v2.4.12 — 2026-05-20

### 추가 — v2.4.11 훅 도입 후속 문서 갱신 (사용자 지적)

- **`README.md` 디렉토리 구조 갱신** — `.claude/hooks/` (pre-pipeline-check, skill-structure-check, post-edit) + `docs/harness/pipeline-overview.md/.html` 추가
- **`pipeline-overview.md/.html` 표 보강** — 강제 행 24개에 `⚙️` 표시 (신규 16 + 운영 8), 범례에 ⚙️ 설명 추가. 코드로 강제되는 진입 조건을 한눈에 식별 가능
- **3부 슬라이드 5** (품질 게이트) — "⚙️ 코드 강제력 (v2.4.11+)" 박스 추가
- **4부 슬라이드 7** (안전망 + 자동 갱신) — 자동 강등 표 다음에 "코드로 강제됨" 한 단락 추가

### 효과

- 새 사용자가 README + pipeline-overview 만 봐도 코드 강제 메커니즘 파악 가능
- 교육 자료에 hook 도입 사실이 반영되어 청중이 "안전망 = 자연어 + 코드" 두 층위로 이해
