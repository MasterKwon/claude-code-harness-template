...
다음 Phase(test) 진입 안전망. **이 Phase 를 건너뛰면 `docs/04.review/reviewed/report.md` 가 비어 있어 test 단계가 진입을 거부합니다.**
...
- **FAIL 시** (High 심각도): `/refine-build-{db|api|screen}` 으로 보완 후 Phase 6 부터 재실행 (build-* 사전 동작이 reviewed/ 자동 무효화)
