...
이 스킬은 API 코드를 새로 생성/갱신하므로, **실행 시작 시 다음 파일이 존재하면 즉시 삭제**합니다:
...
- 라우트 파일이 존재하면 → 읽고 설계서 대비 누락된 엔드포인트만 추가
- 기존 엔드포인트가 설계서와 일치하면 → 건너뜀
- 기존 엔드포인트가 설계서와 다르면 → 해당 부분만 수정 (파일 전체 재작성 금지)
